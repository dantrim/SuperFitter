...
backgrounds="Top,WW,ZV,Zjets,Higgs,Fake,DiscoveryMode_SIG"
...
for sr in ${signalRegions[@]}; do
    for ch in ${leptonChannels[@]}; do
        YieldsTable.py -b -c emCRTop14b,emCRWW14b,emCRZV14b,${ch}${sr} -s ${backgrounds} -w ../results/${sr}_${ch}_SMCwslep0_NoSys/${fitFile} |tee ${ch}${sr}_Disco_bkgYld.log &&
        mv *tex SRBkgYields/ &&

...
ChannelChoices      = ["all","ee","mm","em","tot"]
GridChoices         = ["SMCwslep","SMCwslep0","SMCwslep1","SMCwslep2","SMCwslep3","SMCwslep4","SMCwslep5","SMCwslep6", "SMCwslep7",
...
indirSig    = "/gdata/atlas/dantrim/SusyAna/histoAna/TAnaOutput/SMCwslep/S0_May13/Processed/" # < has ISR uncertainties
...
configMgr.nPoints      = 80  #default is 20 -- increasing for limit rescan
...
if userOpts.doExclusion:
    ...
    for s in sigSamples:
        ...
        if not doToys or hasHippo or hasSig:
            ...
            xsec_tool = xsecUtil()
            xsec_dict = xsec_tool.getXsecUncertainty(userOpts.sigGrid)
            # theory up/down uncertainties
            sigXSSyst = Systematic("SigXSec", configMgr.weights, float(xsec_dict[s][0]), float(xsec_dict[s][1]), "user", "overallSys")
...
#          sigSample.addSystematic(sysObj.AR_all_JVF_MC)
            ...
            # c1c1 w/slep ISR reweighting Herwig++ to MG5 envelope of MG5 uncertainties
            sigSample.addSystematic(sysObj.Signal_C1C1_ISR)

            sigSample.setNormFactor('mu_SIG',1.,0.,10.)
            #sigSample.setNormFactor('mu_SIG',1.,0.,2.) # setting upper bound of scan to 2 for (175.0,75.0) which has signularity at ~2.5
